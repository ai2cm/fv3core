...
def _guard_gpu_report_row(config: Dict, gpu_report_row: List[Any]):
    if len(gpu_report_row) != config[REPORT_KEY_COUNT]:
        raise RuntimeError("Bad row size, is it a gputrace report row?")


def _is_kernel(config: Dict, gpu_report_row: List[Any]) -> bool:
...
def filter_kernel_name(config: Dict, gpu_report_row: List[Any]) -> List[Any]:
...
def filter_kernel_only(config: Dict, gpu_report_row: List[Any]) -> List[Any]:
    _guard_gpu_report_row(config, gpu_report_row)
...
def filter_kernel_time_under_threshold(
    config: Dict, gpu_report_row: List[Any], threshold_in_ns: int
) -> List[Any]:
    if gpu_report_row is None:
        return None
