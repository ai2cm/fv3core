...
class MetricTerms:
    ...
    def divg_u(self):
        if self._divg_u is None:
            self._del6_u, self._del6_v, self._divg_u, self._divg_v = self._calculate_divg_del6()
        return self._divg_u
    ...
    def edge_vect_w(self):
        if self._edge_vect_w is None:
            self._edge_vect_w, self._edge_vect_e, self._edge_vect_s, self._edge_vect_n = self._calculate_edge_a2c_vect_factors()
    ...
    def _set_eta(self):
        ...
        ptop = self._quantity_factory.zeros(
            [], "mb"
        )
        ak = self._quantity_factory.zeros(
            [fv3util.Z_INTERFACE_DIM], "mb"
        )
        bk = self._quantity_factory.zeros(
            [fv3util.Z_INTERFACE_DIM], "mb"
        )
    ...
    def _calculate_center_vectors(self):
        ...
        ec1.data[:-1,:-1,:3], ec2.data[:-1,:-1,:3] = get_center_vector(self._dgrid_xyz, self._grid_type, self._halo,
            self._tile.partitioner, self._rank, self._np
        )
        return ec1, ec2
    ...
    def _calculate_vectors_west(self):
        ...
        ew1.data[1:-1,:-1,:3], ew2.data[1:-1,:-1,:3] = calc_unit_vector_west(
            self._dgrid_xyz, self._agrid_xyz, self._grid_type, self._halo, 
            self._tile.partitioner, self._rank, self._np
        )
        return ew1, ew2
    ...
    def _calculate_more_trig_terms(self, cos_sg, sin_sg):
        ...
        cosa_s = self._quantity_factory.zeros(
        ...
        rsin2 = self._quantity_factory.zeros(
            [fv3util.X_DIM, fv3util.Y_DIM], ""
        )
        ...
        cosa.data[:, :], sina.data[:, :], cosa_u.data[:, :-1], cosa_v.data[:-1, :], cosa_s.data[:-1, :-1], sina_u.data[:, :-1], sina_v.data[:-1, :], rsin_u.data[:, :-1], rsin_v.data[:-1, :], rsina.data[self._halo:-self._halo, self._halo:-self._halo], rsin2.data[:-1, :-1] = calculate_trig_uv(
            self._dgrid_xyz, cos_sg, sin_sg, self._halo, 
            self._tile.partitioner, self._rank, self._np
        )
    ...
    def _init_cell_trigonometry(self):
        ...
        self._cosa_s = self._quantity_factory.zeros(
            [fv3util.X_DIM, fv3util.Y_DIM], ""
        )
        ...
        self._rsin2 = self._quantity_factory.zeros(
            [fv3util.X_DIM, fv3util.Y_DIM], ""
        )
        ...
        cos_sg, sin_sg = calculate_supergrid_cos_sin(
            self._dgrid_xyz, self._agrid_xyz, self._ec1.data[:-1, :-1], self._ec2.data[:-1, :-1], self._grid_type, self._halo, 
            self._tile.partitioner, self._rank, self._np
        )
        
        self._cosa.data[:, :], self._sina.data[:, :], self._cosa_u.data[:, :-1], self._cosa_v.data[:-1, :], self._cosa_s.data[:-1, :-1], self._sina_u.data[:, :-1], self._sina_v.data[:-1, :], self._rsin_u.data[:, :-1], self._rsin_v.data[:-1, :], self._rsina.data[self._halo:-self._halo, self._halo:-self._halo], self._rsin2.data[:-1, :-1] = calculate_trig_uv(
            self._dgrid_xyz, cos_sg, sin_sg, self._halo, 
            self._tile.partitioner, self._rank, self._np
        )

        supergrid_corner_fix(
            cos_sg, sin_sg, self._halo, 
            self._tile.partitioner, self._rank
        )
    ...
    def _calculate_latlon_momentum_correction(self):
        ...
        l2c_v.data[self._halo:-self._halo, self._halo:-self._halo-1], l2c_u.data[self._halo:-self._halo-1, self._halo:-self._halo] = calculate_l2c_vu(
           self._grid.data[:], self._halo, self._np
        )
        return l2c_v, l2c_u
    ...
    def _calculate_xy_unit_vectors(self):
        ...
        ee1.data[self._halo:-self._halo, self._halo:-self._halo, :], ee2.data[self._halo:-self._halo, self._halo:-self._halo, :] = generate_xy_unit_vectors(
    ...
    def _calculate_divg_del6(self):
        ...
        divg_u.data[:-1, :], divg_v.data[:, :-1], del6_u.data[:-1, :], del6_v.data[:, :-1] = calculate_divg_del6(
            sin_sg, self.sina_u.data[:,:-1], self.sina_v.data[:-1,:], 
            self.dx.data[:-1, :], self.dy.data[:, :-1], self.dx_cgrid.data[:, :-1], 
            self.dy_cgrid.data[:-1, :], self._halo, self._tile.partitioner, self._rank
        )
    ...
    def _calculate_grid_z(self):
        z11 = self._quantity_factory.zeros(
            [fv3util.X_DIM, fv3util.Y_DIM], ""
        )
        z12 = self._quantity_factory.zeros(
            [fv3util.X_DIM, fv3util.Y_DIM], ""
        )
        z21 = self._quantity_factory.zeros(
            [fv3util.X_DIM, fv3util.Y_DIM], ""
        )
        z22 = self._quantity_factory.zeros(
            [fv3util.X_DIM, fv3util.Y_DIM], ""
        )
        z11.data[:-1, :-1], z12.data[:-1, :-1], z21.data[:-1, :-1], z22.data[:-1, :-1] = calculate_grid_z(
            self.ec1.data[:-1, :-1], self.ec2.data[:-1, :-1], self.vlon.data[:-1, :-1], self.vlat.data[:-1, :-1], self._np
        )
        return z11, z12, z21, z22

    def _calculate_grid_a(self):
        a11 = self._quantity_factory.zeros(
            [fv3util.X_DIM, fv3util.Y_DIM], ""
        )
        a12 = self._quantity_factory.zeros(
            [fv3util.X_DIM, fv3util.Y_DIM], ""
        )
        a21 = self._quantity_factory.zeros(
            [fv3util.X_DIM, fv3util.Y_DIM], ""
        )
        a22 = self._quantity_factory.zeros(
            [fv3util.X_DIM, fv3util.Y_DIM], ""
        )
        a11.data[:-1, :-1], a12.data[:-1, :-1], a21.data[:-1, :-1], a22.data[:-1, :-1] = calculate_grid_a(
            self.z11.data[:-1, :-1], self.z12.data[:-1, :-1], self.z21.data[:-1, :-1], self.z22.data[:-1, :-1], self.sin_sg5.data[:-1, :-1]
        )
        return a11, a12, a21, a22

    def _calculate_edge_factors(self):
        nhalo = self._halo
        edge_s = self._quantity_factory.zeros(
            [fv3util.X_DIM], ""
        )
        edge_n = self._quantity_factory.zeros(
            [fv3util.X_DIM], ""
        )
        edge_e = self._quantity_factory.zeros(
            [fv3util.Y_DIM], ""
        )
        edge_w = self._quantity_factory.zeros(
            [fv3util.Y_DIM], ""
        )
        edge_w.data[nhalo:-nhalo], edge_e.data[nhalo:-nhalo], edge_s.data[nhalo:-nhalo], edge_n.data[nhalo:-nhalo] = edge_factors(
            self.gridvar.data[:], self.agrid.data[:-1, :-1], self._grid_type, nhalo, 
            self._tile_partitioner, self._rank, RADIUS, self._np
        )
        return edge_w, edge_e, edge_s, edge_n

    def _calculate_edge_a2c_vect_factors(self):
        edge_vect_s = self._quantity_factory.zeros(
            [fv3util.X_DIM], ""
        )
        edge_vect_n = self._quantity_factory.zeros(
            [fv3util.X_DIM], ""
        )
        edge_vect_e = self._quantity_factory.zeros(
            [fv3util.Y_DIM], ""
        )
        edge_vect_w = self._quantity_factory.zeros(
            [fv3util.Y_DIM], ""
        )
        edge_vect_w.data[:-1], edge_vect_e.data[:-1], edge_vect_s.data[:-1], edge_vect_n.data[:-1] = efactor_a2c_v(
            self.gridvar, self.agrid.data[:-1, :-1], self._grid_type, self._halo, 
            self._tile_partitioner, self._rank, RADIUS, self._np
        )