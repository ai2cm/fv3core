...
@utils.stencil()
def dm_stencil(dp: sd, dm: sd, pfull: sd, rf: sd, kindex: sd, dt: float,ptop: float, rf_cutoff_nudge: float, ks: int):
    with computation(PARALLEL), interval(...):
        if pfull < spec.namelist.rf_cutoff:
            rf = compute_rff_vals(pfull, dt, spec.namelist.rf_cutoff, spec.namelist.tau * SDAY, ptop)
...
@utils.stencil()
def ray_fast_wind(wind: sd, rf: sd, dp: sd, dm: sd, pfull: sd, kindex: sd, rf_cutoff_nudge: float, ks: int):
    with computation(FORWARD):
        with interval(0, 1):
            if pfull < spec.namelist.rf_cutoff:
