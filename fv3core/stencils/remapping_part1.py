...
def compute(
    ...
    pe_bottom = utils.make_storage_data(
        utils.repeat(pe[:, :, grid.npz :], pe.shape[2], axis=2), pe.shape
    )
    pe1_bottom = utils.make_storage_data(
        utils.repeat(pe1[:, :, grid.npz :], pe1.shape[2], axis=2), pe1.shape
    )
    ...
    pe_bottom = utils.make_storage_data(
        utils.repeat(pe[:, :, -1:], pe.shape[2], axis=2), pe.shape
    )
    pe1_bottom = utils.make_storage_data(
        utils.repeat(pe1[:, :, -1:], pe.shape[2], axis=2), pe.shape
    )
    pressures_mapu(
        pe,
        pe1,
