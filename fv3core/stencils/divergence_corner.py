...
def compute_diverg_d(div: sd, vf: sd, uf: sd, rarea_c: sd):
    ...
    with computation(PARALLEL), interval(...):
        ...
        with parallel(region[i_start:i_start+1,j_start:j_start+1,:],region[i_end:i_end+1,j_start:j_start+1,:]):
            div = div - vf[0, -1, 0]
        with parallel(region[i_end:i_end+1,j_end:j_end+1],region[i_start:i_start+1,j_end:j_end+1]):
            div = div + vf
        div = rarea_c * div
...
def compute(u, v, ua, va, divg_d):
    ...
    compute_uf( uf, u, va, grid.cos_sg4, grid.cos_sg2, grid.dyc, grid.sin_sg4, grid.sin_sg2,
                origin=(grid.is_ - 1, grid.js, 0),
                domain=(grid.nic + 2, grid.njc + 1, grid.npz),
                splitters={"j_start": j_start, "j_end": j_end} )

    compute_vf( vf, v, ua, grid.cos_sg3, grid.cos_sg1, grid.dxc, grid.sin_sg3, grid.sin_sg1,
    ...
    compute_diverg_d( divg_d, vf, uf, grid.rarea_c, 
                      origin=(grid.is_, grid.js, 0),
                      domain=(grid.nic + 1, grid.njc + 1, grid.npz),
                      splitters={"i_start": i_start, "i_end": i_end, "j_start": j_start, "j_end": j_end} )
