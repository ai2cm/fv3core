...
@gtstencil()
def out(gz_surface: sd, zh: sd, ws: sd, dt: float):
    with computation(BACKWARD):
        with interval(-1, None):
            ws[0, 0, 0] = (gz_surface - zh) * 1.0 / dt
        with interval(0, -1):
            other = zh[0, 0, 1] + DZ_MIN
            zh[0, 0, 0] = zh if zh > other else other


def compute(ndif, damp_vtd, dp0, gz_surface, zh, crx, cry, xfx, yfx, wsd, dt):
    grid = spec.grid
    ...
    gam = utils.make_storage_from_shape(gz_surface.shape, grid.full_origin())
    edge_profile(
        crx,
        xfx,
        crx_adv,
