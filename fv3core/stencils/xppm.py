...
from .yppm import (
    ...
    s14,
    s11,
    pert_ppm_standard_constraint_func,
)
...
def compute_al(q, dxa, iord, is1, ie3, jfirst, jlast, kstart=0, nk=None):
    ...
    if iord < 8:
        ...
        if not grid().nested and spec.config.grid_type < 3:
            if grid().west_edge:
                al_y_edge_0(
...
@gtscript.function
def al_y_edge_0_func(q, dxa):
    return c1 * q[-2, 0, 0] + c2 * q[-1, 0, 0] + c3 * q


@gtscript.function
def al_y_edge_1_func(q, dxa):
    return 0.5 * (
        (
            (2.0 * dxa[-1, 0, 0] + dxa[-2, 0, 0]) * q[-1, 0, 0]
            - dxa[-1, 0, 0] * q[-2, 0, 0]
        ...
        / (dxa[-2, 0, 0] + dxa[-1, 0, 0])
        + ((2.0 * dxa[0, 0, 0] + dxa[1, 0, 0]) * q[0, 0, 0] - dxa[0, 0, 0] * q[1, 0, 0])
        / (dxa[0, 0, 0] + dxa[1, 0, 0])
    )


@gtscript.function
def al_y_edge_2_func(q, dxa):
    return c3 * q[-1, 0, 0] + c2 * q[0, 0, 0] + c1 * q[1, 0, 0]


@utils.stencil()
def get_xflux(xflux: sd, q: sd, c: sd, dxa: sd, iord: int, mord: int):
    """Computes flux in I-direction.

    Dear Fortran developers, please use more descriptive names for us mortals.

    Arguments
    ---------
    xflux (gtscript.Field[I, J, K]): (Output) Flux in I-direction
    q (gtscript.Field[I, J, K]): (Input) Q quantity. Again, wtf?
    c (gtscript.Field[I, J, K]): (Input) One char variables, really?!
    dxa (gtscript.Field[I, J]): (Input) Some sort of grid spacing
    iord (int): Horizontal advection scheme to be applied in the I-direction
    ...
    with computation(PARALLEL), interval(...):
        ...
        if iord < 0:
            al = al if al > 0 else 0.0

        bl, br, b0, tmp = flux_intermediates(q, al, mord)
        fx1 = fx1_fn(c, br, b0, bl)
        xflux = final_flux(c, q, fx1, tmp)  # noqa
...
def get_xflux_order8plus(xflux: sd, q: sd, c: sd, iord: int):
    ...
    with computation(PARALLEL), interval(...):
        ...
        minqj = min(min(q, q[-1, 0, 0]), q[1, 0, 0])
        minmaxq = min(min(abs(xt), maxqj - q), q - minqj)
        dm = sign(minmaxq, xt)

        al = 0.5 * (q[-1, 0, 0] + q) + r3 * (dm[-1, 0, 0] - dm)

        # Do this section only if iord == 8. This is currently checked ouside the stencil
        # {
        xt = 2.0 * dm
        min_aldiff = min(abs(xt), abs(al - q))
        min_aldiffj = min(abs(xt), abs(al[1, 0, 0] - q))
        bl = -1.0 * sign(min_aldiff, xt)
        br = sign(min_aldiffj, xt)
        # }

        # The conditional below should really be:
        ...
        if __INLINED(spec.config.grid_type < 3 and not spec.config.regional):
            ...
            # {
            # iv == 1 here
            pert_ppm_standard_constraint_func(q, bl, br)
            # }

        b0 = get_b0(bl, br)
        fx1 = fx1_fn(c, br, b0, bl)
        xflux = q[-1, 0, 0] + fx1 if c > 0.0 else q + fx1
...
def compute_flux(q, c, xflux, iord, jfirst, jlast, kstart=0, nk=None):
    ...
    if mord not in [5, 6, 7, 8]:
        raise Exception(
            "We have only implemented xppm for hord=5, 6, 7, and 8, not " + str(iord)
        )
