...
class AcousticDynamics:
    ...
    def __init__(
        ...
        # Initialize all halo exchanges required at runtime
        state.__dict__.update(self._temporaries)
        self._zh_halo_update = self.comm.get_scalar_halo_updater(
            [state.zh_quantity], self.grid.halo
        )
        if self.grid.npx == self.grid.npy:
            self._pkc_halo_update = self.comm.get_scalar_halo_updater(
                [state.pkc_quantity], 2
            )
        else:
            self._pkc_halo_update = self.comm.get_scalar_halo_updater(
                [state.pkc_quantity], self.grid.halo
            )

        self._q_con__cappa_halo_update = self.comm.get_scalar_halo_updater(
            [
        ...
        self._q_con_halo_update = self.comm.get_scalar_halo_updater(
            [state.q_con_quantity],
            self.grid.halo,
        )

        self._delp__pt_halo_update = self.comm.get_scalar_halo_updater(
            [
                state.delp_quantity,
                state.pt_quantity,
            ],
        ...
        self._u__v_halo_update = self.comm.get_vector_halo_updater(
            [state.u_quantity], [state.v_quantity], self.grid.halo
        )
        self._w_halo_update = self.comm.get_scalar_halo_updater(
            [state.w_quantity], self.grid.halo
        )
        self._gz_halo_update = self.comm.get_scalar_halo_updater(
            [state.gz_quantity], self.grid.halo
        )
        self._divgd_halo_update = self.comm.get_scalar_halo_updater(
            [state.divgd_quantity], self.grid.halo
        )
        self._heat_source_halo_update = self.comm.get_scalar_halo_updater(
            [state.heat_source_quantity], self.grid.halo
        )
        self._uc__vc_halo_update = self.comm.get_vector_halo_updater(
            [state.uc_quantity], [state.vc_quantity], self.grid.halo
        )
    ...
    def __call__(self, state):
        ...
        if self.do_halo_exchange:
            self._q_con__cappa_halo_update.async_exchange_start()
            self._delp__pt_halo_update.async_exchange_start()
            self._u__v_halo_update.async_exchange_start()
            self._q_con__cappa_halo_update.async_exchange_wait()
        ...
        for it in range(n_split):
            ...
            if not self.namelist.hydrostatic:
                if self.do_halo_exchange:
                    self._w_halo_update.async_exchange_start()
                if it == 0:
                    self._set_gz(
                        self._zs,
                        state.delz,
                    ...
                    if self.do_halo_exchange:
                        self._gz_halo_update.async_exchange_start()
            if it == 0:
                if self.do_halo_exchange:
                    self._delp__pt_halo_update.async_exchange_wait()
            ...
            if self.do_halo_exchange:
                self._u__v_halo_update.async_exchange_wait()
                if not self.namelist.hydrostatic:
                    self._w_halo_update.async_exchange_wait()
            ...
            if self.namelist.nord > 0 and self.do_halo_exchange:
                self._divgd_halo_update.async_exchange_start()
            if not self.namelist.hydrostatic:
                if it == 0:
                    if self.do_halo_exchange:
                        self._gz_halo_update.async_exchange_wait()
                    self._copy_stencil(
                        state.gz,
                        state.zh,
                    )
            ...
            if self.do_halo_exchange:
                self._uc__vc_halo_update.async_exchange_start()
                if self.namelist.nord > 0:
                    self._divgd_halo_update.async_exchange_wait()
                self._uc__vc_halo_update.async_exchange_wait()
            ...
            if self.do_halo_exchange:
                self._delp__pt_halo_update.blocking_exchange()
                self._q_con_halo_update.blocking_exchange()
            ...
            if not self.namelist.hydrostatic:
                ...
                if self.do_halo_exchange:
                    self._zh_halo_update.async_exchange_start()
                    self._pkc_halo_update.async_exchange_start()
            ...
            if not self.namelist.hydrostatic:
                if self.do_halo_exchange:
                    self._zh_halo_update.async_exchange_wait()
                    if self.grid.npx != self.grid.npy:
                        self._pkc_halo_update.async_exchange_wait()
        ...
        if self._do_del2cubed:
            if self.do_halo_exchange:
                self._heat_source_halo_update.blocking_exchange()
