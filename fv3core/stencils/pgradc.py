...
def p_grad_c_stencil(
        uc: FloatField, vc: FloatField, delpc: FloatField, pkc: FloatField, gz: FloatField, rdxc: FloatField, rdyc:FloatField, dt2: float
):
    from __externals__ import local_is, local_ie, local_js, local_je, namelist
    with computation(PARALLEL), interval(...):
        ...
        with horizontal(region[local_is:local_ie+2,local_js:local_je+1]):
            uc = uc + dt2 * rdxc / (wk[-1, 0, 0] + wk) * (
                (gz[-1, 0, 1] - gz) * (pkc[0, 0, 1] - pkc[-1, 0, 0]) + (gz[-1, 0, 0] - gz[0, 0, 1]) * (pkc[-1, 0, 1] - pkc))
