...
class TranslateGridAreas(ParallelTranslateGrid):
    ...
    def compute_sequential(self, inputs_list, communicator_list):
        ...
        for communicator, req in zip(communicator_list, req_list):
            req.wait()
        for state, rank_grid in zip(state_list, self.rank_grids):
...
class TranslateGridGrid(ParallelTranslateGrid):
    ...
    def compute_sequential(self, inputs_list, communicator_list):
        ...
        state_list = []
        for i, inputs in enumerate(inputs_list):
            dgrid = self.grid.quantity_factory.empty(
                dims=[fv3util.X_INTERFACE_DIM, fv3util.Y_INTERFACE_DIM, LON_OR_LAT_DIM],
                units="radians",
            )
            dgrid.data[:] = grid_global.data[:, :, :, i]
            state_list.append({"grid": dgrid})
        req_list = []
        for state, communicator in zip(state_list, communicator_list):
            req_list.append(
...
class TranslateInitGrid(ParallelTranslateGrid):
    ...
    def compute_sequential(self, inputs_list, communicator_list):
        ...
        for i, inputs in enumerate(inputs_list):
            ...
            slice_x, slice_y = tile.subtile_slice(
                rank, grid_section.dims, (npx, npy), overlap=True
            )
        ...
        grid_indexers = []
        for i, state in enumerate(state_list):
            grid_indexers.append(
                GridIndexing.from_sizer_and_communicator(
                    local_sizer, communicator_list[i]
                )
            )
            fill_corners_2d(
                state["grid"].data[:, :, :],
        ...
        for communicator, req in zip(communicator_list, req_list):
            ...
            # to ensure dx and dy mirror across the boundary.
            # Not doing it here at the moment.
        for grid_indexer, state in zip(grid_indexers, state_list):
            state["dx"].data[state["dx"].data < 0] *= -1
            state["dy"].data[state["dy"].data < 0] *= -1
            fill_corners_dgrid(
                state["dx"].data[:, :, None],
        ...
        for i, state in enumerate(state_list):
            ...
            self._compute_local_agrid_part2(
                state, local_quantity_factory, grid_indexers[i]
            )
        req_list = []
        for state, communicator in zip(state_list, communicator_list):
            req_list.append(
        ...
        for i, state in enumerate(state_list):
            self._compute_local_areas_pt1(
                state, communicator_list[i], local_quantity_factory
            )
...
class TranslateUtilVectors(ParallelTranslateGrid):
    ...
    def _compute_local(self, inputs, communicator):
        ...
        (
            state["ec1"].data[:-1, :-1, :3],
            state["ec2"].data[:-1, :-1, :3],
        ) = get_center_vector(
            xyz_dgrid,
            self.grid.grid_type,
            self.grid.halo,
        ...
        (
            state["ew1"].data[1:-1, :-1, :3],
            state["ew2"].data[1:-1, :-1, :3],
        ) = calc_unit_vector_west(
            xyz_dgrid,
            xyz_agrid,
            self.grid.grid_type,
            self.grid.halo,
            communicator.partitioner.tile,
            communicator.rank,
            state["grid"].np,
        )
        (
            state["es1"].data[:-1, 1:-1, :3],
            state["es2"].data[:-1, 1:-1, :3],
        ) = calc_unit_vector_south(
            xyz_dgrid,
            xyz_agrid,
            self.grid.grid_type,
...
class TranslateAAMCorrection(ParallelTranslateGrid):
    ...
    def _compute_local(self, inputs):
        ...
        (
            state["l2c_v"].data[nhalo:-nhalo, nhalo : -nhalo - 1],
            state["l2c_u"].data[nhalo : -nhalo - 1, nhalo:-nhalo],
...
class TranslateMoreTrig(ParallelTranslateGrid):
    ...
    def _compute_local(self, inputs, communicator):
        ...
        nhalo = self.grid.halo
        (
            state["ee1"].data[nhalo:-nhalo, nhalo:-nhalo, :],
            state["ee2"].data[nhalo:-nhalo, nhalo:-nhalo, :],
        ) = generate_xy_unit_vectors(
            xyz_dgrid,
            nhalo,
            communicator.tile.partitioner,
        ...
        (
            ...
            state["cosa_u"].data[:, :-1],
            state["cosa_v"].data[:-1, :],
            state["cosa_s"].data[:-1, :-1],
            state["sina_u"].data[:, :-1],
            state["sina_v"].data[:-1, :],
            state["rsin_u"].data[:, :-1],
            state["rsin_v"].data[:-1, :],
            state["rsina"].data[nhalo:-nhalo, nhalo:-nhalo],
            state["rsin2"].data[:-1, :-1],
        ) = calculate_trig_uv(
            xyz_dgrid,
            cos_sg,
            sin_sg,
...
class TranslateInitCubedtoLatLon(ParallelTranslateGrid):
    ...
    def _compute_local(self, inputs):
        ...
        state["vlon"].data[:-1, :-1], state["vlat"].data[:-1, :-1] = unit_vector_lonlat(
            state["agrid"].data[:-1, :-1], state["agrid"].np
        )
        (
            state["z11"].data[:-1, :-1],
            state["z12"].data[:-1, :-1],
...
class TranslateEdgeFactors(ParallelTranslateGrid):
    ...
    def _compute_local(self, inputs, communicator):
        ...
        nhalo = self.grid.halo
        (
            state["edge_w"].data[nhalo:-nhalo],
            state["edge_e"].data[nhalo:-nhalo],
        ...
        (
            state["edge_vect_w"].data[:-1],
            state["edge_vect_e"].data[:-1],
            state["edge_vect_s"].data[:-1],
            state["edge_vect_n"].data[:-1],
        ) = efactor_a2c_v(
            state["grid"],
            state["agrid"].data[:-1, :-1],
            self.grid.grid_type,
...
class TranslateInitGridUtils(ParallelTranslateGrid):
    ...
    def compute_sequential(self, inputs_list, communicator_list):
        ...
        for state, communicator in zip(state_list, communicator_list):
            req_list.append(
                communicator.start_vector_halo_update(
                    state["divg_v"], state["divg_u"], n_points=self.grid.halo
                )
            )
            req_list.append(
                communicator.start_vector_halo_update(
                    state["del6_v"], state["del6_u"], n_points=self.grid.halo
                )
            )
        for req in req_list:
            req.wait()
    ...
    def _compute_local_eta(self, inputs):
        ...
        state["ptop"] = self.grid.quantity_factory.zeros([], "mb")
        state["ak"] = self.grid.quantity_factory.zeros([fv3util.Z_INTERFACE_DIM], "mb")
        state["bk"] = self.grid.quantity_factory.zeros([fv3util.Z_INTERFACE_DIM], "mb")
        state["ks"], state["ptop"], state["ak"].data[:], state["bk"].data[:] = set_eta(
            npz
        )
        return state
    ...
    def _compute_local_part_1(self, state, communicator):
        ...
        supergrid_corner_fix(
            cos_sg, sin_sg, nhalo, communicator.tile.partitioner, communicator.rank
        )
        for i in range(1, 10):
            state[f"cos_sg{i}"].data[:-1, :-1] = cos_sg[:, :, i - 1]
            state[f"sin_sg{i}"].data[:-1, :-1] = sin_sg[:, :, i - 1]
    ...
    def _compute_local_edges(self, state, communicator):
        ...
        state["edge_vect_s"] = self.grid.quantity_factory.zeros([fv3util.X_DIM], "")
        state["edge_vect_n"] = self.grid.quantity_factory.zeros([fv3util.X_DIM], "")
        state["edge_vect_e"] = self.grid.quantity_factory.zeros([fv3util.Y_DIM], "")
        state["edge_vect_w"] = self.grid.quantity_factory.zeros([fv3util.Y_DIM], "")
        (
            state["edge_vect_w"].data[:-1],
            state["edge_vect_e"].data[:-1],
            state["edge_vect_s"].data[:-1],
