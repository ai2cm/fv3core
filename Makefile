...
FV3_IMAGE ?=$(GCR_URL)/fv3py:latest
...
rundir: #uses the 'serialize.yml' in the fv3gfs-fortran regression tests to configure a test run for generation serialization data
	docker build \
...
tests:
	...
	if [ -z $(shell docker ps -q -f name=$(TEST_DATA_RUN_CONTAINER)) ]; then \
		docker run -d -it --name=$(TEST_DATA_RUN_CONTAINER) -v TestDataVolume$(FORTRAN_VERSION):/test_data $(TEST_DATA_IMAGE) ;fi
