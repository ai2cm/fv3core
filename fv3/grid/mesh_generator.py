...
class MeshGenerator:
    ...
    def __init__(
        ...
        self.lon, self.lat = self.__generate_global_grid(
            grid_type, 2, ntiles, npx, npy, ng, shift_fac=shift_fac
        )
    ...
    def __generate_global_grid(
        self, grid_type, ndims, ntiles, npx, npy, ng, shift_fac=0.0
    ):
        ...
        for n in range(0, ntiles):
            for j in range(0, npy):
                ...
                np.where(
                    grid_global[ng : ng + npx, ng + j, 0, n] < 0.0,
                    grid_global[ng : ng + npx, ng + j, 0, n] + 2.0 * np.pi,
                    grid_global[ng : ng + npx, ng + j, 0, n],
                )
                np.where(
                    np.abs(grid_global[ng : ng + npx, ng + j, 0, n]) < 1.0e-10,
                    0.0,
                    np.abs(grid_global[ng : ng + npx, ng + j, 0, n]),
                )
                np.where(
                    np.abs(grid_global[ng : ng + npx, ng + j, 1, n]) < 1.0e-10,
                    0.0,
                    np.abs(grid_global[ng : ng + npx, ng + j, 1, n]),
                )

        grid_global[ng, ng : npy + ng, :, 1] = grid_global[
            npx - 1 + ng, ng : npy + ng, :, 0
        ]
        grid_global[ng, ng : npy + ng, :, 2] = grid_global[
            npx + ng - 1 : ng - 1 : -1, npy - 1 + ng, :, 0
        ]
        grid_global[ng : npx + ng, npy - 1 + ng, :, 4] = grid_global[
            ng, npy + ng - 1 : ng - 1 : -1, :, 0
        ]
        grid_global[ng : npx + ng, npy - 1 + ng, :, 5] = grid_global[
            ng : npx + ng, ng, :, 0
        ]

        grid_global[ng : npx + ng, ng, :, 2] = grid_global[
            ng : npx + ng, npy - 1 + ng, :, 1
        ]
        grid_global[ng : npx + ng, ng, :, 3] = grid_global[
            npx - 1 + ng, npy + ng - 1 : ng - 1 : -1, :, 1
        ]
        grid_global[npx - 1 + ng, ng : npy + ng, :, 5] = grid_global[
            npx + ng - 1 : ng - 1 : -1, ng, :, 1
        ]

        grid_global[ng, ng : npy + ng, :, 3] = grid_global[
            npx - 1 + ng, ng : npy + ng, :, 2
        ]
        grid_global[ng, ng : npy + ng, :, 4] = grid_global[
            npx + ng - 1 : ng - 1 : -1, npy - 1 + ng, :, 2
        ]

        grid_global[npx - 1 + ng, ng : npy + ng, :, 2] = grid_global[
            ng, ng : npy + ng, :, 3
        ]
        grid_global[ng : npx + ng, ng, :, 4] = grid_global[
            ng : npx + ng, npy - 1 + ng, :, 3
        ]
        grid_global[ng : npx + ng, ng, :, 5] = grid_global[
            npx - 1 + ng, npy + ng - 1 : ng - 1 : -1, :, 3
        ]

        grid_global[ng, ng : npy + ng, :, 5] = grid_global[
            npx - 1 + ng, ng : npy + ng, :, 4
        ]
    ...
    def __mirror_grid(self, grid_global, ng, npx, npy, ndims, ntiles):
        ...
        for nreg in range(1, ntiles):
            for j in range(0, npy):
                ...
                if nreg == 1:
                    ang = -90.0
                    x2, y2, z2 = self.__rot_3d(
                        3, [x1, y1, z1], ang, degrees=True, convert=True
                    )
                elif nreg == 2:
                    ang = -90.0
                    x2, y2, z2 = self.__rot_3d(
                    ...
                    ang = 90.0
                    x2, y2, z2 = self.__rot_3d(
                        1, [x2, y2, z2], ang, degrees=True, convert=True
                    )
                    ...
                    ang = 90.0
                    x2, y2, z2 = self.__rot_3d(
                        2, [x2, y2, z2], ang, degrees=True, convert=True
                    )
                elif nreg == 5:
                    ang = 90.0
                    x2, y2, z2 = self.__rot_3d(
                        2, [x1, y1, z1], ang, degrees=True, convert=True
                    )
                    ang = 0.0
                    x2, y2, z2 = self.__rot_3d(
                        3, [x2, y2, z2], ang, degrees=True, convert=True
                    )
    ...
    def __gnomonic_ed(self, im):
        ...
        for i in range(1, im):
            lon[i, 0], lat[i, 0] = self.__mirror_latlon(
