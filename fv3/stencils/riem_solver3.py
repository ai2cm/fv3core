...
def compute(last_call, dt, akap, cappa, ptop, zs, w, delz, q_con, delp, pt, zh, pe, ppe, pk3, pk, peln, wsd):
    ...
    for j in range(spec.grid.js, spec.grid.je+1):
        ...
        pm2 = (peg[:, kslice_shift] - peg[:, kslice]) / (pelng[:, kslice_shift] - pelng[:, kslice])
        ...
        sim1_solver.solve(spec.grid.is_, spec.grid.ie, dt, gm2, cp2, pe2, dm2, pm2, pem, w2, dz2, pt2, ws2)
