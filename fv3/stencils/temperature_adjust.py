...
@utils.stencil()
def compute_pkz_tempadjust(delp: sd, delz: sd, cappa: sd, heat_source: sd, delt: sd,  pt: sd, pkz: sd):
...
def compute(pt, pkz, heat_source, delz, delp, cappa, n_con, bdt):
    ...
    '''
    compute_pkz_tempadjust(delp, delz, cappa, heat_source, delt, pt, pkz, origin=grid.compute_origin(), domain=(grid.nic, grid.njc, n_con))
