...
def compute(pt, pkz, heat_source, delz, delp, cappa, n_con, bdt):
    ...
    delt = utils.make_storage_data_from_1d(delt_column, dshape, origin=grid.default_origin())
    pkz[isl, jsl, ksl] = np.exp(cappa[isl, jsl, ksl] / (1 - cappa[isl, jsl, ksl]) *
                                np.log(constants.RDG * delp[isl, jsl, ksl] / delz[isl, jsl, ksl] * pt[isl, jsl, ksl]))
