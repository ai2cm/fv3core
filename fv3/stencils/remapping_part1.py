...
@utils.stencil()
def pressure_updates(ps: sd, pe1: sd, pe2: sd, pe: sd, ak: sd, bk: sd, delp: sd, pe_bottom: sd, pn2: sd, peln: sd):
...
@utils.stencil()
def pressures_mapu(pe: sd, pe1: sd, ak: sd, bk: sd, pe_bottom: sd, pe1_bottom: sd, pe0: sd, pe3: sd):
...
def compute(qvapor, qliquid, qrain, qsnow, qice, qgraupel, qcld, pt, delp, delz, peln, u, v, w, ua, cappa, q_con, pkz, pk, pe, hs, te, ps, wsd, omga, ak, bk, gz, cvm, ptop, akap, r_vir,):
    grid = spec.grid
    ...
    if spec.namelist['kord_tm'] < 0:
        if hydrostatic:
            ...
            moist_cv.compute_pt(qvapor, qliquid, qice, qrain, qsnow, qgraupel, q_con,
                                gz, cvm, pt, cappa, delp, delz, r_vir)
    if not hydrostatic:
        delz_adjust(delp, delz, origin=grid.compute_origin(), domain=grid.domain_shape_compute())
    pe_bottom = utils.make_storage_data(np.repeat(pe.data[:, :, grid.npz:], pe.shape[2], axis=2), pe.shape)
    ...
    if not hydrostatic:
        ...
        undo_delz_adjust(delp, delz, origin=grid.compute_origin(), domain=grid.domain_shape_compute())
    #if do_omega:  # NOTE untested
    #    pe3 = cp.copy(omga, (grid.is_, grid.js, 1))

    pe0 = cp.copy(peln, grid.compute_origin(), domain=(grid.nic, grid.njc, grid.npz + 1))
    peln = cp.copy(pn2, grid.compute_origin(), domain=(grid.nic, grid.njc, grid.npz + 1))
    if hydrostatic:
        # pkz
        pass
    else:
        moist_cv.compute_pkz(qvapor, qliquid, qice, qrain, qsnow,
                             qgraupel, q_con, gz, cvm, pkz, pt,
                             cappa, delp, delz, r_vir)
    
    #if do_omega:
    # dp2 update, if larger than pe0 and smaller than one level up, update omega and  exit
   
    pe_bottom = utils.make_storage_data(np.repeat(pe.data[:, :, -1:], pe.shape[2], axis=2), pe.shape)
    pe1_bottom = utils.make_storage_data(np.repeat(pe1.data[:, :, -1:], pe.shape[2], axis=2), pe.shape)
    pressures_mapu(pe, pe1, ak, bk, pe_bottom, pe1_bottom, pe0, pe3, origin=grid.compute_origin(), domain=domain_jextra)
    #  map_ppm.compute(u, pe0, pe3, gz, -1, spec.namelist['kord_mt'])
    domain_iextra = (grid.nic + 1, grid.njc, grid.npz + 1)
    pressures_mapv(pe, ak, bk, pe_bottom, pe0, pe3, origin=grid.compute_origin(), domain=domain_iextra)
