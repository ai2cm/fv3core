...
def compute(u, v, w, dp, pfull, dt, ptop, ks):
    grid = spec.grid
    rff_initialized = False  # TODO pull this out to higher level so don't do over and over
    rf_cutoff = spec.namelist["rf_cutoff"]
    rf_cutoff_nudge = rf_cutoff + min(100., 10. * ptop)
    if not rff_initialized:
        # is only a column actually
        rf = np.ones(grid.npz)
        rffvals = ray_super.rayleigh_rfvals(dt, spec.namelist["tau"] * ray_super.SDAY, rf_cutoff, pfull, ptop)
        ...
        neg_pfull = np.argwhere(pfull[spec.grid.is_, spec.grid.js, 0 : ks] < rf_cutoff_nudge)
        if len(neg_pfull) == 0:
            k_rf = 1
        else:
            k_rf = neg_pfull[-1][-1] + 1
        
        rff_initialized = True  # TODO propagate to rest of dyncore so this isn't unecessarily redone
    ...
    dm_stencil(dp, dm, origin=grid.compute_origin(), domain=(grid.nic+1, grid.njc+1, k_rf))
    dm = utils.make_storage_data(np.squeeze(dm[:, :, k_rf - 1]), dm.shape)
    ray_fast_u(u, rf, dp, dmu, origin=grid.compute_origin(), domain=(grid.nic, grid.njc + 1, kmax))
    ray_fast_v(v, rf, dp, dmv, origin=grid.compute_origin(), domain=(grid.nic+1, grid.njc, kmax))
    ray_fast_w(w, rf, spec.namelist['hydrostatic'], origin=grid.compute_origin(),domain=(grid.nic, grid.njc, kmax))
    dmu = utils.make_storage_data(np.squeeze(dmu[:, :, kmax - 1]), dm.shape)
    dmv = utils.make_storage_data(np.squeeze(dmv[:, :, kmax - 1]), dm.shape)
    ray_fast_u_dm(u, dmu, dm, origin=grid.compute_origin(),domain=(grid.nic, grid.njc + 1, k_rf))
    ray_fast_v_dm(v, dmv, dm, origin=grid.compute_origin(),domain=(grid.nic+1, grid.njc, k_rf))
