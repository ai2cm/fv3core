...
def geoadjust_ut(ut: sd, dy: sd, sin_sg3: sd, sin_sg1: sd, dt2: float):
    with computation(PARALLEL), interval(...):
        ut[0, 0, 0] = dt2 * ut * dy * sin_sg3[-1, 0, 0] if ut > 0 else dt2 * ut * dy * sin_sg1


@utils.stencil()
def geoadjust_vt(vt: sd, dx: sd, sin_sg4: sd, sin_sg2: sd, dt2: float):
    with computation(PARALLEL), interval(...):
        vt[0, 0, 0] = dt2 * vt * dx * sin_sg4[0, -1, 0] if vt > 0 else dt2 * vt * dx * sin_sg2
...
def compute(delp, pt, u, v, w, uc, vc, ua, va, ut, vt, divgd, dt2):
    ...
    geoadjust_ut(ut, grid.dy, grid.sin_sg3, grid.sin_sg1, dt2, origin=geo_origin, domain=(grid.nic + 3, grid.njc + 2, grid.npz))
    geoadjust_vt(vt, grid.dx, grid.sin_sg4, grid.sin_sg2, dt2, origin=geo_origin, domain=(grid.nic + 2, grid.njc + 3, grid.npz))
    ...
    absolute_vorticity(vort, grid.fC, grid.rarea_c, origin=grid.compute_origin(), domain=(grid.nic + 1, grid.njc + 1, grid.npz))
