...
@gtscript.stencil(backend=utils.backend)
def zh_damp_stencil(area: sd, z2: sd, fx: sd, fy: sd, ra_x: sd, ra_y: sd, fx2: sd, fy2: sd, rarea: sd, zh: sd):
    with computation(PARALLEL), interval(...):
...
def column_calls(zh, crx_adv, cry_adv, xfx_adv, yfx_adv, ra_x, ra_y, column_namelist):
    ...
    if column_namelist['damp'] > 1e-5:
        ...
        z2, fx, fy = fvtp2d.compute_no_sg(z2, crx_adv, cry_adv, spec.namelist['hord_tm'], xfx_adv, yfx_adv, ra_x, ra_y)
        fx2, fy2, wk, z2 = delnflux.compute_no_sg(z2, fx2, fy2, column_namelist['ndif'], column_namelist['damp'], wk)
        zh_damp_stencil(grid.area, z2, fx, fy, ra_x, ra_y, fx2, fy2, grid.rarea, zh,
