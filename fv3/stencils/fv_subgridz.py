...
from fv3.utils.global_constants import CP_AIR, CV_AIR, CP_VAP, CV_VAP, C_LIQ, C_ICE, GRAV, ZVIR, RDGAS
...
def standard_cm(cpm, cvm, q0_vapor, q0_liquid, q0_rain, q0_ice, q0_snow, q0_graupel):
    ...
    cpm = (1. - (q0_vapor + q_liq + q_sol)) * CP_AIR + q0_vapor * CP_VAP + q_liq * C_LIQ + q_sol * C_ICE
    cvm = (1. - (q0_vapor + q_liq + q_sol)) * CV_AIR + q0_vapor * CV_VAP + q_liq * C_LIQ + q_sol * C_ICE
    return cpm, cvm
...
@utils.stencil()
def init(den: sd, gz: sd, gzh: sd, t0: sd, pm: sd, u0: sd, v0: sd, 
         w0: sd, hd: sd, cvm: sd, cpm: sd, te: sd, ua: sd, va: sd, w: sd, ta: sd, peln: sd,
         delp: sd, delz: sd, q0_vapor: sd, q0_liquid: sd, q0_rain: sd, q0_ice: sd, q0_snow: sd,
         q0_graupel: sd, xvir: float):
...
@utils.stencil()
def compute_qcon(qcon: sd, q0_liquid: sd, q0_ice: sd, q0_snow: sd, q0_rain: sd, q0_graupel: sd):
    with computation(PARALLEL), interval(...):
        qcon = qcon_func(qcon, q0_liquid, q0_ice, q0_snow, q0_rain, q0_graupel)
        
@utils.stencil()               
def recompute_qcon(ri: sd, ri_ref: sd, qcon: sd, q0_liquid: sd, q0_rain: sd, q0_ice: sd, q0_snow: sd, q0_graupel: sd):
    with computation(BACKWARD), interval(...):
        if ri[0, 0, 1] < ri_ref[0, 0, 1]:
            qcon = qcon_func(qcon, q0_liquid, q0_ice, q0_snow, q0_rain, q0_graupel)
        
@utils.stencil()
def m_loop(ri: sd, ri_ref: sd, pm: sd, u0: sd, v0: sd, w0: sd, t0: sd, hd: sd, gz: sd, qcon: sd, delp: sd, pkz: sd, q0_vapor: sd, pt1: sd, pt2: sd, tv2: sd, t_min: float, t_max: float, ratio: float, xvir: float):
    with computation(BACKWARD), interval(...):#interval(1, None): -- from doing the full stencil
        tv1 = t0[0, 0, -1] * (1. + xvir * q0_vapor[0, 0, -1] - qcon[0, 0, -1])
        tv2 = t0 * (1. + xvir * q0_vapor - qcon)
        pt1 = tv1 / pkz[0, 0, -1]
        pt2 = tv2 / pkz
...
'''
@utils.stencil()
def KH_instability_adjustment_bottom(ri: sd, ri_ref: sd, mc: sd, q0: sd, delp: sd, h0: sd):
    with computation(BACKWARD), interval(...):
        if ri < ri_ref:
            h0 = mc * (q0 - q0[0, 0, -1])
            q0 = q0 - h0 / delp
...
def KH_instability_adjustment(ri, ri_ref, mc, q0, delp, h0, origin=None, domain=None):
    KH_instability_adjustment_bottom(ri, ri_ref, mc, q0, delp, h0, origin=origin, domain=domain)
    KH_instability_adjustment_top(ri, ri_ref, mc, q0, delp, h0, origin=(origin[0], origin[1], origin[2] - 1), domain=domain)
# special case for total energy that may be a fortran bug
def KH_instability_adjustment_te(ri, ri_ref, mc, q0, delp, h0, hd, origin=None, domain=None):
    KH_instability_adjustment_bottom_te(ri, ri_ref, mc, q0, delp, h0, hd, origin=origin, domain=domain)
    KH_instability_adjustment_top(ri, ri_ref, mc, q0, delp, h0, origin=(origin[0], origin[1], origin[2] - 1), domain=domain)
@utils.stencil()
def KH_instability_adjustment_bottom_te(ri: sd, ri_ref: sd, mc: sd, q0: sd, delp: sd, h0: sd, hd: sd):
    with computation(BACKWARD), interval(...):
        if ri < ri_ref:
            h0 = mc * (hd - hd[0, 0, -1])
            q0 = q0 - h0 / delp
@utils.stencil()
def double_adjust_cvm(cvm: sd, cpm:sd, gz: sd, u0: sd, v0: sd, w0: sd, hd: sd, t0: sd, te: sd, q0_liquid: sd, q0_vapor: sd, q0_ice: sd, q0_snow: sd, q0_rain: sd, q0_graupel: sd):
    with computation(BACKWARD), interval(...):
        cpm, cvm = standard_cm(cpm, cvm, q0_vapor, q0_liquid, q0_rain, q0_ice, q0_snow, q0_graupel)
        tv = tvol(gz, u0, v0, w0)
        t0 = (te - tv) / cvm
...
@utils.stencil()
def fraction_adjust(t0: sd, ta:sd, u0: sd, ua: sd, v0: sd, va: sd, w0: sd, w: sd, fra: float, hydrostatic: bool):
    with computation(PARALLEL), interval(...):
...
def compute(state, nq, dt):
    ...
    if k_bot < min(grid.npz, 24):
        ...
        t_max = T3_MAX
    if state.pe[grid.is_, grid.js, 0] < 2.:
        t_min = T1_MIN
    else:
        t_min = T2_MIN
    ...
    origin = grid.compute_origin()
    init(den, gz, gzh, t0, pm, u0, v0, w0, hd, cvm, cpm, te,
         state.ua, state.va, state.w, state.pt, state.peln,
         state.delp, state.delz, q0["qvapor"], q0["qliquid"],
         q0["qrain"], q0["qice"], q0["qsnow"], q0["qgraupel"], xvir, origin=origin, domain=kbot_domain)
    '''
    ...
    for n in range(1):
        ...
        for k in range(k_bot - 1, 0, -1):
            ...
            m_loop(ri, ri_ref, pm, u0, v0, w0, t0, hd, gz, qcon, state.delp, state.pkz, q0["qvapor"], pt1, pt2, tv2,
                   t_min, t_max, ratio, xvir, origin=korigin, domain=kdomain)
            ...
            equivalent_mass_flux(ri, ri_ref, mc, state.delp, ratio, origin=korigin, domain=kdomain)
            for tracername in utils.tracer_variables:
                KH_instability_adjustment(ri, ri_ref, mc, q0[tracername], state.delp, h0,origin=korigin, domain=kdomain)
            if k == k2d+1:
                print('input tracers', qcon[i2d, j2d, k2d], q0["qliquid"][i2d, j2d, k2d], q0["qrain"][i2d, j2d, k2d], q0["qice"][i2d, j2d, k2d], q0["qsnow"][i2d, j2d, k2d], q0["qgraupel"][i2d, j2d, k2d])
              
            recompute_qcon(ri, ri_ref, qcon, q0["qliquid"], q0["qrain"], q0["qice"], q0["qsnow"], q0["qgraupel"], origin=korigin_m1, domain=kdomain)
            if k == k2d+1:
                print('after input tracers', qcon[i2d, j2d, k2d], q0["qliquid"][i2d, j2d, k2d], q0["qrain"][i2d, j2d, k2d], q0["qice"][i2d, j2d, k2d], q0["qsnow"][i2d, j2d, k2d], q0["qgraupel"][i2d, j2d, k2d])
            ...
            KH_instability_adjustment(ri, ri_ref, mc, u0, state.delp, h0,origin=korigin, domain=kdomain)
            ...
            if n == 0 and k == k2d:
                ...
                for ki in range(k, k_bot):
                    ...
                    comp = u0[i2d, j2d, ki]
                    if ref != comp:
                        print("BROKE",ki, ri[i2d, j2d, ki]< ri_ref[i2d, j2d, ki], ri[i2d, j2d,  ki], ri_ref[i2d, j2d, ki], 'h0',h0[i2d, j2d, ki],state.delp[i2d, j2d, ki],'mc', mc[i2d,j2d, ki],comp, ref, ref- comp)
                        print(pt1[i2d, j2d, ki], pt2[i2d, j2d, ki],  tv2[i2d, j2d, ki],  t0[i2d, j2d, ki])
                    else:
                        print(ki,ri[i2d, j2d, ki]< ri_ref[i2d, j2d, ki], ri[i2d, j2d,  ki], ri_ref[i2d, j2d, ki], 'h0',  h0[i2d, j2d, ki],state.delp[i2d, j2d, ki],'mc', mc[i2d,j2d, ki], ref)
                '''
                for i in range(grid.is_, grid.ie + 1):
            ...
            KH_instability_adjustment(ri, ri_ref, mc, v0, state.delp, h0, origin=korigin, domain=kdomain)
            KH_instability_adjustment(ri, ri_ref, mc, w0, state.delp, h0, origin=korigin, domain=kdomain)
            KH_instability_adjustment_te(ri, ri_ref, mc, te, state.delp, h0, hd, origin=korigin, domain=kdomain)
            if k == k2d + 1:
                print('double trouble',  t0[i2d, j2d, k2d+1],  t0[i2d, j2d, k2d])
            double_adjust_cvm(cvm, cpm, gz, u0, v0, w0, hd, t0, te, q0["qliquid"], q0["qvapor"], q0["qice"], q0["qsnow"], q0["qrain"], q0["qgraupel"], origin=korigin_m1, domain=kdomain_m1)
            if k == k2d + 1:
                print('after double trouble',  t0[i2d, j2d, k2d+1],  t0[i2d, j2d, k2d])
    if fra < 1.:
        fraction_adjust(t0, state.pt, u0, state.ua, v0, state.va, w0, state.w, fra, spec.namelist['hydrostatic'], origin=origin, domain=kbot_domain)
        for tracername in utils.tracer_variables:
            fraction_adjust_tracer(q0[tracername], state.tracers[tracername], fra, origin=origin, domain=kbot_domain)
    for tracername in utils.tracer_variables:
        cp.copy_stencil(q0[tracername], state.tracers[tracername], origin=origin, domain=kbot_domain)
    finalize(u0, v0, w0, t0, state.ua, state.va, state.pt, state.w, state.u_dt, state.v_dt, rdt, origin=origin, domain=kbot_domain)
