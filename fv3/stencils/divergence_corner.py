...
def compute(u, v, ua, va, divg_d):
    ...
    if spec.namelist['grid_type'] == 4:
       ...
       if grid.north_edge:
          compute_uf_edge_values( uf, u, grid.dyc, grid.sin_sg4, grid.sin_sg2, origin=(grid.is_-1,grid.je+1,0), domain=edge_domain )
    ...
    if spec.namelist['grid_type'] == 4:
       ...
       compute_diverg_d( divg_d, vf, uf, origin=(grid.is_,grid.js,0), domain=(grid.nic+1,grid.njc+1,grid.npz) )
       if grid.sw_corner:
       ...
       basic.adjustmentfactor_stencil( grid.rarea_c, divg_d, origin=(grid.is_,grid.js,0), domain=(grid.ie,grid.je,grid.npz) )
