...
def solve(is_, ie, dt, gm2, cp2, pe2, dm, pm2, pem, w2, dz2, ptr, wsr):
    ...
    for i in range(nic):
        dz2[i, km] = -dm[i, km] * constants.RDGAS * ptr[i, km] * np.exp((cp2[i, km] - 1.0) * np.log(max(spec.namelist['p_fac'] * pm2[i, km], p1[i] + pm2[i, km])))
              
    for k in range(npz - 2, -1, -1):
        for i in range(nic):
            p1[i] = (pe2[i, k] + bb[i, k] * pe2[i, k+1] + g_rat[i, k] * pe2[i, k+2]) * 1.0 / 3.0 - g_rat[i, k] * p1[i]
            dz2[i, k] = -dm[i, k] * constants.RDGAS * ptr[i, k] * np.exp((cp2[i, k] - 1.0) * np.log(max(spec.namelist['p_fac'] * pm2[i, k], p1[i] + pm2[i, k])))
