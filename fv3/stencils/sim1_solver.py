...
def solve(is_, ie, dt, gm2, cp2, pe2, dm, pm2, pem, w2, dz2, ptr, wsr, p_fac):
    ...
    # w solver
    aa[:, slice_m2] = t1g * 0.5 * (gm2[:, slice_n] + gm2[:, slice_m2])\
                                   / (dz2[:, slice_n] + dz2[:, slice_m2]) * (pem[:, slice_m2] + pp[:, slice_m2])
    ...
    for k in range(1, km):
        for i in range(nic):
            ...
            w2[i, k] = (dm[i, k] * w1[i, k] +
                        dt * (pp[i, k+1] - pp[i, k]) - aa[i, k] * w2[i, k-1]) / bet[i]
  
    for i in range(nic):
        p1[i] = t1g * gm2[i, km] / dz2[i, km] * (pem[i, km+1] + pp[i, km+1])
        gam[i, km] = aa[i, km] / bet[i]
        bet[i] = dm[i, km] - (aa[i, km] + p1[i] + aa[i, km] * gam[i, km])
        w2[i, km] = (dm[i, km] * w1[i, km] + dt * (pp[i, km+1] - pp[i, km]) - p1[i] * wsr[i, 0] - aa[i, km] * w2[i, km - 1])/ bet[i]
    ...
    pe2[:, 0] = 0.0
    for k in range(npz):
        pe2[:, k+1] = pe2[:, k] + dm[:, k] * (w2[:, k] - w1[:, k]) / dt

    p1[:] = (pe2[:, km] + 2.0 * pe2[:, km + 1]) * 1.0 / 3.0
  
    for i in range(nic):
        dz2[i, km] = -dm[i, km] * constants.RDGAS * ptr[i, km] * np.exp((cp2[i, km] - 1.0) * np.log(max(p_fac * pm2[i, km], p1[i] + pm2[i, km])))
               
    for k in range(npz - 2, -1, -1):
        for i in range(nic):
            p1[i] = (pe2[i, k] + bb[i, k] * pe2[i, k+1] + g_rat[i, k] * pe2[i, k+2]) * 1.0 / 3.0 - g_rat[i, k] * p1[i]
            dz2[i, k] = -dm[i, k] * constants.RDGAS * ptr[i, k] * np.exp((cp2[i, k] - 1.0) * np.log(max(p_fac * pm2[i, k], p1[i] + pm2[i, k])))
