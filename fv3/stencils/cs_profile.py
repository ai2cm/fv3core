...
def set_vals_0(gam:sd, q:sd, delp:sd, a4_1:sd, qs:sd):
    ...
    with computation(FORWARD):
        with interval(1,-1):
            ...
            bet = 2. + grid_ratio + grid_ratio - gam
            q = (3. * (a4_1[0,0,-1] + a4_1[0,0,0]) - q[0,0,-1]) / bet
...
def set_vals_1(gam:sd, q:sd, delp:sd, a4_1:sd, qs:sd):
    with computation(PARALLEL):
        with interval(0,1):
            ...
            q = ((grid_ratio + grid_ratio) * (grid_ratio + 1.) * a4_1 + a4_1[0,0,1]) / bet
            gam = (1. + grid_ratio*(grid_ratio + 1.5)) / bet
    with computation(FORWARD):
        with interval(1,-1):
            #set middle
            d4 = delp[0,0,-1] / delp
    ...
    with computation(PARALLEL):
        with interval(-1,None):
            #set bottom
            a_bot = 1. + (delp[0,0,-2] / delp[0,0,-1])*((delp[0,0,-2] / delp[0,0,-1])+1.5)
            q = (2.*(delp[0,0,-2] / delp[0,0,-1])*((delp[0,0,-2] / delp[0,0,-1])+1.)*a4_1[0,0,-1]+a4_1[0,0,-2]-a_bot*q[0,0,-1]) / ( (delp[0,0,-2] / delp[0,0,-1])*((delp[0,0,-2] / delp[0,0,-1])+0.5) - a_bot*gam[0,0,-1] )
...
def compute(qs, a4_1, a4_2, a4_3, a4_4, delp, km, i1, i2, iv, kord):
    ...
    set_inner_as(a4_1, a4_2, a4_3, a4_4, gam, extm, ext5, ext6, kord, origin=(i1,grid.js,2), domain=(i_extent, 1, km-4))
    set_bottom_as(a4_1, a4_2, a4_3, a4_4, extm, iv, origin=(i1,grid.js,km-1), domain=(i_extent, 1, 2))
    ...
    a4_1, a4_2, a4_3, a4_4 = cs_limiters.compute(a4_1, a4_2, a4_3, a4_4, extm, 2, i1, i_extent, km-3, 1)
    a4_1, a4_2, a4_3, a4_4 = cs_limiters.compute(a4_1, a4_2, a4_3, a4_4, extm, 1, i1, i_extent, km-3, 1)
