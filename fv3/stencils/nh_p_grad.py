...
def applya2b(pp: sd, pk3: sd, gz: sd, delp: sd, wk1: sd, top_value: float):
    with computation(PARALLEL):
        with interval(0, 1):
            ...
            pk3[0, 0, 0] = top_value
        with interval(1, -1):
            a2b_ord4.compute(pp, wk1, replace=True)
            a2b_ord4.compute(pk3, wk1, replace=True)
        with interval(...):
            a2b_ord4.compute(gz, wk1, replace=True)
        with interval(0,-2):
...
def CalcU(u: sd, du: sd, dt: float, wk: sd, wk1: sd, gz: sd, pk3: sd, pp:sd, rdx: sd):
    with computation(PARALLEL), interval(...):
        ...
        du = dt / (wk[0, 0, 0] + wk[1, 0, 0]) * ( (gz[0, 0, 1] - gz[1, 0, 0]) * (pk3[1, 0, 1] - pk3[0, 0, 0]) + (gz[0, 0, 0] - gz[1, 0, 1]) * (pk3[0, 0, 1] - pk3[1, 0, 0]) )
        #nonhydrostatic contribution
        u[0, 0, 0] = (u[0, 0, 0] + du[0, 0, 0] + dt/(wk1[0, 0, 0] + wk1[1, 0, 0]) * ((gz[0, 0, 1] - gz[1, 0, 0]) * (pp[1, 0, 1]-pp[0, 0, 0]) + (gz[0, 0, 0] - gz[1 ,0 , 1]) * (pp[1 ,0 ,1] - pp[1, 0, 0]))) * rdx[0, 0, 0]

@utils.stencil()
def CalcV(v: sd, dv: sd, dt: float, wk: sd, wk1: sd, gz: sd, pk3: sd, pp:sd, rdy: sd):
    with computation(PARALLEL), interval(...):
        #hydrostatic contribution
        dv[0, 0, 0] = dt / (wk[0, 0, 0] + wk[0, 1, 0]) * ((gz[0, 0, 1] - gz[0, 1, 0]) * (pk3[0, 1, 1] - pk[0, 0, 0]) + (gz[0, 0, 0] - gz[0, 1, 1]) * (pk3[0, 0, 1] - pk3[0, 1, 0]))
        #nonhydrostatic contribution
        v[0, 0, 0] = (v[0, 0, 0] + dv[0, 0, 0] + dt / (wk1[0, 0, 0] + wk1[0, 1, 0]) * ((gz[0, 0, 1] - gz[0, 1, 0]) * (pp[0, 1, 1] - pp[0, 0, 0]) + (gz[0, 0, 0] - gz[0, 1, 1]) * (pp[0, 0, 1] - pp[0, 1, 0]))) * rdy[0, 0, 0]
...
def compute(u, v, pp, gz, pk3, delp, dt, ptop, akap):
    ...
    applya2b(pp, pk3, gz, delp, wk1, top_value, origin=orig, domain=(grid.nic+1, grid.njc+1, grid.npz+1))
    ...
    # do j=js,je+1
    # do i=is,ie
    CalcU(u, du, dt, wk, wk1, gz, pk3, pp, grid.rdx, origin=orig, domain=(grid.nic, grid.njc+1, grid.npz))

    dv = utils.make_storage_from_shape(v.shape, origin=orig)
    # do j=js,je
    # do i=is,ie+1
    CalcV(v, dv, dt, wk, wk1, gz, pk3, pp, grid.rdy, origin=orig, domain=(grid.nic+1, grid.njc, grid.npz))
