    ...
    for j in range(grid.js, grid.je):
        for i in range(grid.is_, grid.ie + 1):
            ...
            for k in range(grid.npz - 1, -1, -1):
                if q[i, j, k] < 0.0 and sum1 > 0.:
                    dq = min(sum1, -q[i, j, k] * dp[i, j, k])
                    sum1 -= dq
                    sum2 += dq
                    q[i, j, k] += dq / dp[i, j, k]
            for k in range(grid.npz - 1, -1, -1):
                if q[i, j, k] > 0.0 and sum2 > 0.:
...
'''
@utils.stencil()
def fix_neg_water(pt: sd, dp: sd, delz: sd, qv: sd, ql: sd, qr: sd, qs: sd, qi: sd, qg: sd, lv00: float, d0_vap: float):
    with computation(PARALLEL), interval(...):
        ...
        dq = 0.
        qv, qi, qs, qg, qr, ql, pt = fix_negative_ice(qv, qi, qs, qg, qr, ql, pt, lcpk, icpk, dq)
        qv, qi, qs, qg, qr, ql, pt = fix_negative_liq(qv, qi, qs, qg, qr, ql, pt, lcpk, icpk, dq)
