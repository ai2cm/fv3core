    ...
    def state_from_inputs(self, inputs: dict, grid=None) -> dict:
        ...
        for name, properties in self.inputs.items():
            ...
            if len(properties["dims"]) > 0:
                ...
                # this was removed in merge (replacing above to start of "if") does it need to be put back?
                # remove this or put it back in before merging PR
    ...
    def state_from_inputs(self, inputs: dict, grid=None) -> dict:
        ...
        for name, properties in self.inputs.items():
            ...
            if len(properties["dims"]) > 0:
                ...
                input_slice = _serialize_slice(state[standard_name], properties.get("n_halo", utils.halo))
