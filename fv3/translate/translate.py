...
class TranslateFortranData2Py:
    ...
    def make_storage_data(
            self, array, istart=0, jstart=0, kstart=0, dummy_axes=None, axis=2, names_4d=None,
    ):
    ...
    def slice_output(self, inputs, out_data=None):
        ...
        for var in self.out_vars.keys():
            ...
            if isinstance(data_result, dict):
                names_4d = info.get("names_4d", utils.tracer_variables)
                var4d = np.zeros((ds['iend'] - ds['istart'] + 1, ds['jend'] - ds['jstart'] + 1, ds['kend'] - ds['kstart'] + 1,len(data_result)))
                for varname, data_element in data_result.items():
                    index = names_4d.index(varname)
                    var4d[:, :, :, index] = np.squeeze(data_element.data[self.grid.slice_dict(ds)])
